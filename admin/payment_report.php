...
<!--    Sales-->
    <div class="row">
            <div class="col-md-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <p class="card-title mb-0">PAYMENT REPORT</p><hr>
        <div class="row">
        <div class="card">
        <div class="card-body">
    ...
    if(!empty ($from) && !empty ($to)){
      ...
      $qry= "SELECT amount FROM payment";
        $amount_query = mysqli_query($connection, $qry);
        $number_of_amount = mysqli_num_rows($amount_query);

			$query1 = "SELECT * FROM payment WHERE payment_date	BETWEEN '$from' AND '$to'";
        ...
        $result = mysqli_query($connection, "SELECT SUM(amount) AS value_sum FROM payment WHERE payment_date BETWEEN '$from' AND '$to'"); 
        ...
        $query = "SELECT * FROM payment WHERE payment_date BETWEEN '$from' AND '$to' LIMIT $page_1, $perpage";
        $sales_query = mysqli_query($connection, $query);
		$number_of_rows = mysqli_num_rows($sales_query);
        
echo "<b>Number of Payment(s): {$number_of_rows} Results</b>";
echo "<br>";
echo "<br>";
echo "<b>Report of Payments from {$from} to {$to}</b>";
echo "<br>";
echo "<br>";
echo "<table class='css-serial table table-hover table-bordered table-striped table-responsive'>"; 
		
echo "<tr><th>Customer Name</th><th>Customer Email</th><th>Phone Number</th><th>Amount</th><th>Reference No</th><th>Status</th><th>Payment Date</th></tr>";
		
while($row = mysqli_fetch_array($sales_query)){ 
echo "<tr><td>" . $row['customer_name'] . "</td><td>" . $row['customer_email'] . "</td><td>" . $row['phone_number'] . "</td><td>" .'&#8358;'. $row['amount'] . "</td><td>" . $row['reference'] . "</td><td>" . $row['status'] . "</td><td>" . $row['payment_date'] . "</td></tr>";  
}
...
-->
                ...
                for($i=1; $i<=$total; $i++){
                if($i == $page){
                echo "<li><a href='payment_report?page={$i}&from={$from}&to={$to}&submit='>&nbsp;&nbsp;<button type='button' class='btn btn-outline-dark btn-icon'>{$i}</button>&nbsp;&nbsp;</a></li>";
                }else{
                echo "<li><a href='payment_report?page={$i}&from={$from}&to={$to}&submit='>&nbsp;&nbsp;<button type='button' class='btn btn-outline-dark btn-icon'>{$i}</button>&nbsp;&nbsp;</a></li>"; 
                }
                }
